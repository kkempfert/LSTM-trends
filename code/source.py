...
# Calculate the loss for the "orig" variant
def calc_loss_orig(y_true, y_pred):
    num_K = int(K.int_shape(y_pred)[1]/3)
    if (num_K == 1):
        pred_p = crop(1, 0, num_K)(y_pred) # first num_K column(s) of output
        pred_alpha = crop(1, num_K, 2*num_K)(y_pred) # second num_K column(s) of output
        pred_beta = crop(1, 2*num_K, 3*num_K)(y_pred) # third num_K column(s) of output
        ...
        nonzero_alpha = tf.boolean_mask(alpha, mask)
        nonzero_beta = tf.boolean_mask(beta, mask)
        ...
        for i in range(num_K):
            ...
            alphak = crop(1, num_K + i, num_K + i + 1)(y_pred)
            betak = crop(1, num_K*2 + i, num_K*2 + i + 1)(y_pred)
            
            nonzero_alphak = tf.boolean_mask(alphak, mask)
            nonzero_betak = tf.boolean_mask(betak, mask)
            ...
            if (i == (num_K - 1)):
                ...
                #nonzero_like = nonzero_like + nonzero_p_remain * K.pow(nonzero_betak, nonzero_alphak) * K.pow(nonzero_y, (nonzero_alphak - 1)) * K.exp(-nonzero_betak*nonzero_y)/tf.exp(tf.math.lgamma(nonzero_alphak))
            else:
                nonzero_like = nonzero_like + nonzero_p_remain * nonzero_pk * gamma.prob(nonzero_y) # likelihood using Keras density 
                #nonzero_like = nonzero_like + nonzero_p_remain * nonzero_pk * K.pow(nonzero_betak, nonzero_alphak) * K.pow(nonzero_y, (nonzero_alphak - 1)) * K.exp(-nonzero_betak*nonzero_y)/tf.exp(tf.math.lgamma(nonzero_alphak))
...
# Calculate the loss for the "reparam-round" variant
def calc_loss_reparam_round(y_true, y_pred):
    num_K = int(K.int_shape(y_pred)[1]/3)
    if (num_K == 1):
        pred_p = crop(1, 0, num_K)(y_pred) # first num_K column(s) of output
        pred_mu = crop(1, num_K, 2*num_K)(y_pred) # second num_K column(s) of output
        ...
        mask2 = K.equal(y, 0)
        zero_p = tf.boolean_mask(p, mask2)
        nonzero_loss = -K.log(nonzero_p) -K.log(gamma.cdf(nonzero_y + epsilon) - gamma.cdf(nonzero_y - epsilon))
        ...
        for i in range(num_K):
            ...
            #prior_beta = prior_beta -.5 * K.log(2.0) + K.log(K.sqrt(beta_var)) + .5*K.log(np.pi) + K.pow(nonzero_betak, 2)/(2.0*beta_var)
            
            if (i == (num_K - 1)):
                nonzero_like = nonzero_like + nonzero_p_remain * (gamma.cdf(nonzero_y + epsilon) - gamma.cdf(nonzero_y - epsilon))
            else:
                nonzero_like = nonzero_like + nonzero_p_remain * nonzero_pk * (gamma.cdf(nonzero_y + epsilon) - gamma.cdf(nonzero_y - epsilon))
...
# Calculate the loss for the "orig-round" variant
def calc_loss_orig_round(y_true, y_pred):
    num_K = int(K.int_shape(y_pred)[1]/3)
    if (num_K == 1):
        pred_p = crop(1, 0, 1)(y_pred) # first num_K column(s) of output
        pred_alpha = crop(1, 2, 3)(y_pred) # second num_K column(s) of output
        pred_beta = crop(1, 4, 5)(y_pred) # third num_K column(s) of output
        y = tf.keras.backend.cast(y_true, dtype = data_type)
        p = pred_p
        alpha = pred_alpha
        beta = pred_beta
...
# Main function for fitting the LSTM
def fit_lstm(train, batch_size, epoch, neurons, timesteps, stateful, learning_rate, file_begin, file_end, opt, dropout, num_K):
    # Set up some file names
    filename_loss = file_begin + "loss" + file_end + ".csv"
    filename_train = file_begin + "train" + file_end + ".csv"
    
    dim_output = num_K * 3
    
    # Grab indices of observations to keep track of data
    inds_train = train[:, train.shape[1] - 1]
    train = np.delete(train, train.shape[1] - 1, 1)
    
    # Grab missing indicators, which will be used for loss calculations 
    ...
    # Creating a missing value indicator for the forecast stage
    if (covariates_NA):
    ...
    # Begin training over the epochs
        ...
        if np.isfinite(train_loss):
            ...
            if early_stopping:
                if (train_loss <= min(train_loss_vec)):
                    tf.keras.models.save_model(model, file_begin + "model" + file_end)
                else:
                    # For early stopping, if the training loss exceeds the best achieved loss, add to the counter.
                    # When the counter reaches the specified patience level, training is stopped. 
                    counter_ES = counter_ES + 1 
                    if counter_ES >= patience:
                        return train_preds, train_loss_vec
            else:
                tf.keras.models.save_model(model, file_begin + "model" + file_end)
            train_preds_prev = train_preds
             # If enabled, write out the results every so often. 
            if write_every:
                    train_preds.to_csv(file_begin + "train" + file_end + "_" + str(e) + ".csv", index = False)
            elif (write_out and e!= 0):
                if (e % counter_writeout == 0):
                    write_loss(train_loss_vec, filename_loss)
                    train_preds.to_csv(file_begin + "train" + file_end + ".csv", index = False)
        else:
            if e == 0:
                ...
                return train_preds_prev, train_loss_vec


    return train_preds, train_loss_vec #, weights
...
# Main function to generate from the fitted LSTM
def generate_call(model, data, batch_size, neurons, timesteps, filename, stateful = True, num_K = 2):
    dim_output = num_K * 3 
    if (stateful):
        model.layers[1].reset_states(states)
    
    dim_input = data.shape[1] - 1 - timesteps
    ...
    gen_vec = list()
    pred_vec = list()
    gen_len = data.shape[0]
    for i in range(gen_len):
        if (i == 0):
            y_vec = data[i, dim_input:(data.shape[1] - 1)]  # grabs the lagged values of y (excludes covariates and y_t)
            data_i = np.concatenate((data[i, 0:dim_input], y_vec)).reshape(1, data.shape[1] - 1)
        else:
            # Remove first observation from y_vec and add on the most recent generated observation to y_vec
            y_vec = np.concatenate((np.delete(y_vec, 0), np.array([gen_vec[i-1]])))
            data_i = np.concatenate((data[i, 0:dim_input], y_vec)).reshape(1, data.shape[1] - 1)
        prediction = forecast_lstm(model, data_i, timesteps, neurons)
        if (num_K == 1):
            gen_i = draw_mixture(prediction[0, 0],
                                 prediction[0, 1],
                                 prediction[0, 2], 
                                 num_K = num_K)
        else:
            gen_i = draw_mixture(list(itertools.chain(*prediction[:, 0:num_K])), 
                                 list(itertools.chain(*prediction[:, num_K:(2*num_K)])),
                                 list(itertools.chain(*prediction[:, (2*num_K):(3*num_K)])),
                                 num_K = num_K)
        pred_vec.append(prediction[0, 0:num_vars])   
        gen_vec.append(gen_i)
        
    
    predictions_arr = np.reshape(list(itertools.chain(*pred_vec)), (gen_len, num_vars))
    predictions_df = pd.DataFrame(data = predictions_arr, columns = colnames)
    generated_df = pd.DataFrame(gen_vec, columns=["gen"])
    df = pd.concat([generated_df, predictions_df], axis = 1)
    if (filename != ""):
...
def write_time(time_vec, filename):
    time_df = pd.DataFrame(data = time_vec, columns = ["sec"])
    time_df.to_csv(filename, index = False)
...
# Calculate the modified loss (for the "round" variants) after training
def log_likelihood_round(y, output1, output2, output3, num_K):
    if "orig" in variant:
        p = output1
        alpha = output2
        beta = output3
    ...
    if (num_K == 1):
        if (y > 0):
            if (alpha == 0.0):
                ...
                loss = -np.log(p) - np.log(gamma.cdf(y + epsilon).numpy() - gamma.cdf(y - epsilon).numpy())
        else:
            loss = -np.log(1 - p)
    else:
        if (y > 0):
            if (alpha[0] == 0.0 and alpha[1] == 0.0):
                ...
                gamma1 = tfp.distributions.Gamma(concentration = alpha[0], rate = beta[0])
                gamma2 = tfp.distributions.Gamma(concentration = alpha[1], rate = beta[1])
                loss = -np.log((1 - p[0]) * p[1] * (gamma1.cdf(y + epsilon).numpy() - gamma1.cdf(y - epsilon).numpy()) +
                               (1 - p[0]) * (1 - p[1]) * (gamma2.cdf(y + epsilon).numpy() - gamma2.cdf(y - epsilon).numpy())) 
        else:
            loss = -np.log(p[0])
    return loss


# Draw values from the mixture when generating samples
def draw_mixture(output1, output2, output3, num_K):
    if "orig" in variant:
        p = output1
        alpha = output2
        beta = output3
        ...
        beta = [mu[i]/var[i] for i in range(len(mu))]
    if (num_K == 1):
        ind = binom.rvs(n = 1, p = p, size = 1)
        if (ind[0] > 0):
            val = gamma.rvs(a = alpha, scale = 1/beta, size=1)[0]
        else:
            val = 0
    else:
        ind = binom.rvs(n = 1, p = 1 - p[0], size = 1)
        if (ind[0] > 0):
            which_gamma = binom.rvs(n = 1, p = 1 - p[1], size = 1)[0]
            val = gamma.rvs(a = alpha[which_gamma], scale = 1/beta[which_gamma], size=1)[0]
        else:
            val = 0
    return(val)
...
# Obtain a generated sample from the fitted LSTM
def generate_lstm(seed_m, model, mfilename, series, timesteps, batch_size, epochs, neurons, stateful, learning_rate, name, opt, dropout, num_K):
    # Form lagged data that will be used as input to the LSTM. 
    # Note that only the beginning part of the data (the first timesteps rows) will be used for generation, as conditioning values.
    train = setup_data(series, timesteps, gen = True)    
    
    # Remove the inds column of the data, as we don't need that for generating from the LSTM. 
    train = np.delete(train, train.shape[1] - 1, 1)
    ...
    # (Note that here, we only need to trim the training set. 
    ...
    # Set seed m then run generator
    np.random.seed(seed_m)
    
    # Generate from model 
    gen_bestt = generate_call(model, train, batch_size, neurons, timesteps, 
                                 mfilename, stateful, num_K)
    

# Fit the LSTM
def run_lstm(series, timesteps, batch_size, epochs, neurons, stateful, learning_rate, name, opt, dropout, num_K):
    # Form lagged data that will be used as input to the LSTM
    train = setup_data(series, timesteps)       
    print(train.shape)
    ...
    if (param_seed_b > 1):
        # Bootstrap values 
        # Draw blocks of length block_len = 365
        block_len = 365
        block_num = math.floor(train.shape[0]/block_len)   # use floor, so that if sample size is not divisible by block_len, we simply omit the last observations that don't fit into a block. 
        inds = np.random.choice(block_num, block_num)    # draw blocks independently and with equal probability with replacement 
        
        blocks_l = list(range(0 * block_len, block_num * block_len, block_len)) 
        inds_boot = list()
        for i in range(len(blocks_l)):
            inds_boot.append(list(range(blocks_l[inds[i]], blocks_l[inds[i]] + block_len)))
            
        inds_boot = list(itertools.chain(*inds_boot))
        
        bootstrapped = train[list(inds_boot)]
        train = bootstrapped
   ...
   #     test = test[:-remainder, :] # remove the observation(s) from the end of the time series
    ...
    # Set other file names that correspond to thfe specific trial  
    file_end = gfile_end + "b%d_v%d" % (param_seed_b, param_seed_v)  
    file_begin = output_path + name + "_"
    
    filename_loss = file_begin + "loss" + file_end  + ".csv"
    filename_train = file_begin + "train" + file_end + ".csv"
    filename_time = file_begin + "time" + file_end + ".csv"
    print(filename_train) 
    
    
    # Fit model 
    begin = time.time()
    lstm_model = fit_lstm(train, batch_size, epochs, neurons, timesteps, stateful, learning_rate, file_begin, file_end, opt, dropout, num_K)
    end = time.time()
    
    total_time = end - begin
    write_time([total_time], filename_time)
    
    train_preds = lstm_model[0]
    train_loss = lstm_model[1]

    train_preds.to_csv(filename_train, index = False)
    write_loss(train_loss, filename_loss)

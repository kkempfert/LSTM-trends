...
output_path = dir_path + "output/hpc/"
...
series = np.array(station["prcp"])
names = ["prcp"] # (can be multi-dimensional)
...
gfile_end = construct_filename(param_timesteps, param_batch_size, param_epochs, param_neurons, stateful, param_learning_rate, param_opt, param_dropout, param_num_K)   
 # Set other file names that correspond to the specific trial 
...
for m in param_seed_m:
    ...
    mfile_end = gfile_end + "b%d_v%d_m%d" % (param_seed_b, param_seed_v, m)   # specific file end for generation index m
